...
categories: NLP
tag: "deeq NLP" python grpc baikalai 바이칼AI
...
파이썬에서 새로운 라이브러리나 모듈을 요리조리 뜯어볼 때 쓰는 톡톡 건드릴 때는 '.'을 찍고 `<TAB>`을 쳐주면 됩니다.
...
* pos: 형태소 분석 결과 옵니다. 여러 문장일 때, 문장 구분이 없는 조금 곤란한 결과를 가져옵니다.
* morphs: 형태소만 꺼내옵니다.
